...
**A command runs one time only.** It cannot be attached to a recurring job, because a repeated collection is not a job. Schedule it with a one-time job, or with the *Schedule one-time job* action on a machine page.

Both pages also need a view permission, because a scheduling action needs the job to decide and the form has not picked one yet. The configuration page needs `turbo.view_onetimejob`. The machine page needs `turbo.view_enrolledmachine`: Zentral knows the configuration only after it resolves the serial number, so the scheduling action cannot be the first gate, and the response must not tell the user if a serial number is enrolled.
...
That role can schedule a `sysdiagnose` and nothing else. It still opens the *Schedule one-time job* page, which the view permission gates, and the picker there offers the kinds that the policy permits. The submit is what applies the policy. Zentral hides the action only when **no** kind could be permitted.
...
|`digests`|Optional. The storage digests the config asked for, keyed by algorithm, in lower-case hex like `sha256`. Required for an artifact large enough for [multipart](#multipart), which nothing else can validate.|
...
|`400`|`{"error": "invalid_digests"}`|`digests` is not an object, or one of its values is not a hex digest.|
