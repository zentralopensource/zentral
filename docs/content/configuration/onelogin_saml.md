...
|---|---|
|RelayState|Default relay state. Only available if IdP initiated login is checked. Make sure there are no whitespaces!|
...
See [PBAC](pbac.md) for the access-control model, and [Realm Groups and Roles](sso.md#realm-groups) for mapping Realm Groups to Roles.
