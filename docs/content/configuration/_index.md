 ...
 * [`password_reset_handler`](password_reset_handler.md)
 * [`secret_engines`](secret_engines.md)
 * [`users`](users.md)
 * `queues`
 * `actions`
 * `apps`
...
The `apps` section holds the configuration of each Zentral app, keyed by the app module name. The [event stores](stores.md) are configured there, under `apps.zentral.core.stores`.
