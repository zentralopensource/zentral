...
* `zentral_mdm_devices_per_realm_group`  
Number of enrolled devices per realm group, in the same buckets as `zentral_mdm_devices` (see above). Devices are linked to realm groups via the realm user of their enrollment sessions, and only direct memberships are taken into account — parent groups are not included. A device is counted once in each group its realm user is a member of, so a sum over the `realm_group` label can be greater than the total number of devices. The `realm` and `realm_group` labels are the name of the realm and the display name of the group. Both are set to `_` for the devices enrolled without realm authentication, and for the devices whose realm user is not a member of any group. The `blueprint` and `platform` labels are the same as for `zentral_mdm_devices`.
