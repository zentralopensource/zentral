...
This boolean is used to toggle the MDM metrics endpoint for Prometheus. `false` by default. The endpoint is served at `/metrics/mdm/`, and the scraper must authenticate with the token set in the `metrics_bearer_token` key of the `api` section. When activated, five different metric families are exported:

* `zentral_mdm_enrollment_sessions`
Number of enrollment sessions. This family is a counter, and is published as `zentral_mdm_enrollment_sessions_total`. The `type` label is one of `DEP`, `OTA`, `USER`, or `RE` for the re-enrollment sessions. The available `status` label values depend on the type: `PHASE_1`, `PHASE_2`, `PHASE_3` (OTA only), `ACCOUNT_DRIVEN_START`, `ACCOUNT_DRIVEN_AUTHENTICATED` (user enrollments only), `STARTED`, `AUTHENTICATED` (first MDM check-in `Authenticate` call), and `COMPLETED` (first MDM check-in `TokenUpdate` call). The `realm` label is the name of the realm of the session realm user, or `_` for the sessions without realm authentication.
* `zentral_mdm_commands`
Number of MDM commands. This family is a counter, and is published as `zentral_mdm_commands_total`. The `channel` label is either `device` or `user`. The `name` label is the name of the command, for example `DeviceInformation`. The `status` label is one of `Acknowledged`, `CommandFormatError`, `Error`, `NotNow`, or `_` for a command the target has not responded to yet. The `artifact` and `version` labels carry the name of the artifact and the version of the artifact version for the commands installing or removing an artifact, and are set to `_` for all the other commands.
* `zentral_mdm_devices`
Number of enrolled devices. Multiple buckets are published, corresponding to the number of days within which the devices contacted the server for the last time. The `le` label (*less than or equal to*) can be used to select each bucket. Available values are 1, 7, 14, 30, 45, 90 (days), and +Inf (*infinity*) for the bucket including all the devices, including the ones that never contacted the server. The `blueprint` label is the name of the blueprint of the device, or `_` if it has none. The `platform` label is one of `iOS`, `iPadOS`, `macOS`, or `tvOS`. The `supervised` label is `true`, `false`, or `_` if the device has not reported it. The `blocked` label is `true` for the devices that have been blocked.
* `zentral_mdm_enrolled_device_bucket`
Number of enrolled devices per realm group, in the same buckets as `zentral_mdm_devices` (see above). Devices are linked to realm groups via the realm user of their latest enrollment session, and the parent groups are included, like for the realm group tag mappings. A device is counted once in each group its realm user is a member of, directly or indirectly, so a sum over the `realm_group` label can be greater than the total number of devices. The `realm` and `realm_group` labels are the name of the realm and the display name of the group. Both are set to `_` for the devices whose latest enrollment session has no realm user, and for the devices whose realm user is not a member of any group. The `blueprint` and `platform` labels are the same as for `zentral_mdm_devices`.
* `zentral_mdm_target_artifacts_bucket`
Number of artifact versions reported by the devices and the users, in buckets corresponding to the number of days within which the device or the user contacted the server for the last time (see above). The `name`, `type`, and `version` labels describe the artifact version, with `type` being one of `Activation`, `Asset`, `Certificate Asset`, `Configuration`, `Configuration (manual)`, `Data Asset`, `Enterprise App`, `Profile`, `Provisioning Profile`, or `Store App`. The `channel` label is either `Device` or `User`. The `status` label is one of `Acknowledged`, `AwaitingConfirmation`, `Installed`, `Uninstalled`, `Failed`, `RemovalFailed`, or `ForceReinstall`. The `present` label is `true` for the statuses meaning that the artifact version is on the target (`Acknowledged` and `Installed`), `false` for the other ones, and `_` for an unknown status.
    ...
    "results": [
        {
            ...
            "ethernet_mac_address": "14:98:77:aa:bb:02",
            "wifi_mac_address": "14:98:77:aa:bb:03",
