...
from .utils import TurboAPITestCase, force_configuration, force_enrollment, force_recurring_job
...
class TurboConfigurationAPITestCase(TurboAPITestCase):
    ...
    def test_list_configurations(self):
        ...
        configurations = [force_configuration(name=f"{p}{get_random_string(8)}") for p in ("a", "b")]
        ...
        self.assertEqual([c["id"] for c in response.json()["results"]],
                         [str(c.pk) for c in configurations])
    ...
    def test_delete_configuration_with_schedule_blocked(self):
        # a recurring/one-time job cascades from the configuration, so deletion is blocked while any
        # schedule references it (not just enrollments)
