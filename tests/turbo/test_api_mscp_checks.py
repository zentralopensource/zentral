...
from .utils import (TurboAPITestCase, force_configuration, force_mscp_check,
...
class TurboMSCPCheckAPITestCase(TurboAPITestCase):
    ...
    def test_create_mscp_check(self, post_event):
        ...
        self.assertEqual(mscp_check.compliance_check.name, f"{rule_id} = int(15)")
    ...
    def test_list_mscp_checks(self):
        # ordered by rule_id — LimitOffset pagination over an unordered queryset returns unstable pages.
        # lowercase prefixes so the expected order does not depend on the database collation
        mscp_checks = [force_mscp_check(rule_id=f"{p}{get_random_string(8)}") for p in ("a", "b")]
        ...
        self.assertEqual([c["id"] for c in response.json()["results"]],
                         [str(c.pk) for c in mscp_checks])

    def test_list_mscp_checks_filter_by_configuration(self):
        # a check's configuration is reached through its Job's schedules — both the recurring and the
        # one-time path count; an unscheduled check is excluded
        configuration = force_configuration()
        recurring_check = force_mscp_check()
        force_recurring_job(configuration=configuration, job=recurring_check.job)
        one_time_check = force_mscp_check()
        force_one_time_job(configuration=configuration, job=one_time_check.job)
        force_mscp_check()  # unscheduled
        self.set_permissions("turbo.view_mscpcheck")
        response = self.get(reverse("turbo_api:mscp_checks"), {"configuration": str(configuration.pk)})
        self.assertEqual(response.status_code, 200)
        self.assertEqual({r["id"] for r in response.json()["results"]},
                         {str(recurring_check.pk), str(one_time_check.pk)})
    ...
    def test_update_mscp_check_bumps_version(self):
        ...
        self.assertEqual(mscp_check.compliance_check.name, f"{mscp_check.rule_id} = int(20)")
