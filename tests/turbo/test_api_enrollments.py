...
class TurboEnrollmentAPITestCase(TurboAPITestCase):
    ...
    def test_list_enrollments_annotates_machine_count(self):
        ...
        make_enrolled_machine(enrollment)
        other_enrollment = force_enrollment(meta_business_unit=self.mbu)  # no machines
        self.set_permissions("turbo.view_enrollment")
        with CaptureQueriesContext(connection) as ctx:
            response = self.get(reverse("turbo_api:enrollments"))
        self.assertEqual(response.status_code, 200)
        # pk order — LimitOffset pagination over an unordered queryset returns unstable pages
        self.assertEqual([e["id"] for e in response.json()["results"]],
                         [enrollment.pk, other_enrollment.pk])
