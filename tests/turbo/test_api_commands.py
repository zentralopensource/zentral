...
class TurboCommandAPITestCase(TurboAPITestCase):
    ...
    def test_create_sysdiagnose_without_kwargs(self):
        # a kind with no options has nothing to send, so an absent kwargs block is normal
        ...
        # Command.save() creates the Job with kind == backend: the two are one wire identity
    ...
    def test_create_command_ignores_another_backends_kwargs(self):
        # A client that sends the wrong block gets the command, and the API drops the block.
        # The backend selects its own kwargs.
        self.set_permissions("turbo.add_command")
        response = self.post(reverse("turbo_api:commands"),
    ...
    def test_create_command_audit_event(self, post_event):
        ...
        # the audit event carries the backend, which a policy for one kind uses
    ...
    def test_update_command_kwargs_bumps_the_version(self):
        # The version is on the Job, and a new version starts a new run. It changes when the
        # work of the agent changes.
        command = force_command(backend=CommandBackend.FILE_EXPORT,
                                backend_kwargs={"patterns": ["/tmp/a.log"], "max_size": 1024})
    ...
    def test_update_command_name_only_keeps_the_version(self):
        # a new name does not change the work of the agent, so it must not start a new run
        command = force_command(backend=CommandBackend.FILE_EXPORT,
                                backend_kwargs={"patterns": ["/tmp/a.log"], "max_size": 1024})
        self.set_permissions("turbo.change_command")
        response = self.put(reverse("turbo_api:command", args=(command.pk,)),
    ...
    def test_update_command_backend_immutable(self):
        # The backend is the kind, and the kind is part of the wire identity. A new backend
        # changes what the agent runs under the same pk and version.
    ...
    def test_delete_command(self):
        ...
        # the delete removes the Job too, like a Script
