...
class TurboSetupOneTimeJobsTestCase(TurboSetupTestCase):
    ...
    def test_create_one_time_job_unknown_configuration_redirects_when_anonymous(self):
        # The view reads the configuration lazily, so no code reads the database before the
        # authorization mixin runs. An anonymous request for an absent pk gets the login
        # redirect, and learns nothing about the configurations.
    ...
    def test_create_one_time_job_needs_the_view_permission(self):
        # createOneTimeJob needs the job, which the form has not picked yet, so it cannot gate the
        # page. turbo.view_onetimejob does, and the scheduling actions alone are not enough.
        configuration = force_configuration()
        self.login_with_policy(turbo_policy(self.group))
        response = self.client.get(reverse("turbo:create_one_time_job", args=(configuration.pk,)))
        self.assertEqual(response.status_code, 403)
    ...
    def test_create_one_time_job_get(self):
        configuration = force_configuration()
        self.login_with_policy(turbo_policy(self.group, "turbo.view_onetimejob"))
        response = self.client.get(reverse("turbo:create_one_time_job", args=(configuration.pk,)))
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, "turbo/onetimejob_form.html")
    ...
    # PBAC: a forbid on the kind of the job refuses the schedule, although the role has the
    # action. The forbid is above the broad permit of turbo_policy(). This is one of the two
    # shapes that work. The other is a permit for one kind, alone. See test_pbac.
    ...
    def test_create_one_time_job_form_offers_only_the_allowed_kinds(self):
        # The picker shows only the kinds that a policy permits. An operator with a permit for
        # one kind then does not get a 403, with the loss of the form.
        configuration = force_configuration()
        ...
        # The queryset does not change, so a forged post reaches the typed check.
