...
class TurboStatusPublicTestCase(TurboPublicTestCase):
    ...
    def test_status_unknown_kind_holds_the_row_and_skips_the_entry(self):
        # The third part of the rolling upgrade. The machine holds a job that a newer instance
        # served, and posts its status to an older instance. definition_wire_ref() reads .pk on
        # None, which gives a 500 for the whole batch. Zentral must still write the tracker, or
        # the next sweep marks a job that the agent holds as removed. The entry has the kind,
        # which the wire accepts as a short string.
        configuration, _, serial_number, token = self._enrolled()
