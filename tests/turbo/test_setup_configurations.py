...
from .utils import (TurboSetupTestCase, force_configuration, force_enrollment, force_recurring_job,
                    make_enrolled_machine)
...
class TurboSetupConfigurationsTestCase(TurboSetupTestCase):
    ...
    def test_configuration_detail_enrollments_not_loaded_without_permission(self):
        # the enrollments (with their secrets) are only fetched for a user allowed to view them
        enrollment = force_enrollment(meta_business_unit=self.mbu)
        configuration = enrollment.configuration
        self.login("turbo.view_configuration")  # no turbo.view_enrollment
        response = self.client.get(configuration.get_absolute_url())
        self.assertEqual(response.status_code, 200)
        self.assertNotIn("enrollments", response.context)
        self.login("turbo.view_configuration", "turbo.view_enrollment")
        response = self.client.get(configuration.get_absolute_url())
        self.assertEqual([e.pk for e, _, _ in response.context["enrollments"]], [enrollment.pk])
    ...
    def test_delete_configuration_with_schedule_404(self):
        # a configuration with a schedule can't be deleted either — the jobs would cascade
        recurring_job = force_recurring_job()
        self.login("turbo.delete_configuration")
        response = self.client.get(
            reverse("turbo:delete_configuration", args=(recurring_job.configuration.pk,)))
        self.assertEqual(response.status_code, 404)

    def test_configuration_detail_delete_button_hidden_with_schedule(self):
        recurring_job = force_recurring_job()
