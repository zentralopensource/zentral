...
from zentral.contrib.turbo.models import (Configuration, OneTimeJobMachine, RecurringJob,
                                          RecurringJobMachine, Script)
...
class TurboModelStrTestCase(TestCase):
    ...
    def test_recurring_job_machine_str(self):
        rj = force_recurring_job()
        rjm = RecurringJobMachine.objects.create(serial_number="C02X", recurring_job=rj)
        self.assertEqual(str(rjm), f"{rj} on C02X")

    def test_one_time_job_machine_str(self):
        otj = force_one_time_job()
        otjm = OneTimeJobMachine.objects.create(serial_number="C02X", one_time_job=otj)
        self.assertEqual(str(otjm), f"{otj} on C02X")
...
class TurboMSCPCheckComplianceCheckNameTestCase(TestCase):
    def test_typed_odv_names_do_not_collide(self):
        # odv_int=1, odv_string="1" and odv_bool=True are three different identities: the ODV is
        # rendered with its type so the derived ComplianceCheck names — unique on (model, name) —
        # cannot collide
        rule_id = get_random_string(12)
        int_check = force_mscp_check(rule_id=rule_id, odv_int=1)
        string_check = force_mscp_check(rule_id=rule_id, odv_string="1")
        bool_check = force_mscp_check(rule_id=rule_id, odv_bool=True)
        self.assertEqual(int_check.compliance_check.name, f"{rule_id} = int(1)")
        self.assertEqual(string_check.compliance_check.name, f"{rule_id} = string(1)")
        self.assertEqual(bool_check.compliance_check.name, f"{rule_id} = bool(true)")
