...
class TurboSetupCommandsTestCase(TurboSetupTestCase):
    # The console is read-only for commands, like for stores.Store and probes.Action. An
    # operator defines a command through the API, where the kwargs serializer validates it.
    ...
    def test_sysdiagnose_command_detail(self):
        ...
        # a kind with no options shows no kwargs, and one artifact
    ...
    def test_command_detail_lists_its_schedules(self):
        # The detail page lists the schedules that run the definition. This is the reason for
        # the page: every job list links the get_absolute_url of a definition.
        configuration = force_configuration()
        command = force_command()
        one_time_job = force_one_time_job(configuration=configuration, job=command.job)
        self.login("turbo.view_command", "turbo.view_onetimejob")
        response = self.client.get(reverse("turbo:command", args=(command.pk,)))
        self.assertEqual(response.status_code, 200)
    ...
    def test_machine_one_time_job_form_offers_a_command(self):
        # the one-time paths accept the kind that the recurring path refuses
        command = force_command()
