...
class SantaSyncSessionTestCase(TestCase):
    ...
    def synced_rule_count(self, enrolled_machine):
        return MachineRule.objects.filter(enrolled_machine=enrolled_machine, cursor__isnull=True).count()
    ...
    # the machine rules are acknowledged when the client asks for the next batch,
    # but the client only writes them once the whole download is over
    @expectedFailure
    ...
    # the batch acknowledged during the aborted session is recorded as synced, so it is never
    # sent again. The in flight batch is discarded when the next session starts, and the client
    # keeps some rules, so neither the cleanup nor a clean sync comes to the rescue.
    @expectedFailure
    ...
    # the machine rules are deleted during the preflight, the client only clears its
    # own database during the rule download
    @expectedFailure
    def test_interrupted_clean_sync_keeps_the_client_and_the_server_in_sync(self):
        client, _, targets = self.force_client(batch_size=2, rule_count=6)
        client.sync()
        ...
        # and the server must still agree
        client.preflight()
        client.enrolled_machine.refresh_from_db()
        self.assertTrue(client.enrolled_machine.sync_ok())
    ...
    # the machine rules are deleted during the preflight, but santa returns before its
    # own cleanup when it receives no rule
    @expectedFailure
