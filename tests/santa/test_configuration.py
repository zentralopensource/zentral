...
from .utils import fake_enrolled_machine, force_realm
...
class SantaConfigurationTestCase(TestCase):
    ...
    def test_blocked_path_regex_default_allowed_path_regex(self):
        ...
        sync_server_config = config.get_sync_server_config(fake_enrolled_machine(get_random_string(12)), (1, 14))
    ...
    def test_non_matching_path_regex_stable_across_preflights(self):
        ...
        first = config.get_sync_server_config(fake_enrolled_machine(serial_number), (1, 14))
        second = config.get_sync_server_config(fake_enrolled_machine(serial_number), (1, 14))
    ...
    def test_enable_all_event_upload_sync_server_config_0(self):
        ...
        self.assertEqual(
            config.get_sync_server_config(fake_enrolled_machine("111111"), (2022, 1))["enable_all_event_upload"],
            False
        )
        self.assertEqual(
            config.get_sync_server_config(fake_enrolled_machine("777777"), (2022, 1))["enable_all_event_upload"],
            False
        )
    ...
    def test_event_detail_local_source_distributes_nothing(self):
        ...
        sync_server_config = config.get_sync_server_config(fake_enrolled_machine(get_random_string(12)), (2022, 1))
    ...
    def test_event_detail_voting_portal_source_unavailable_removes_the_button(self):
        config = Configuration.objects.create(name=get_random_string(256),
                                              event_detail_source=Configuration.EventDetailSource.VOTING_PORTAL)
        with self.assertLogs("zentral.contrib.santa.models", level="ERROR") as cm:
            self.assertEqual(config.get_event_detail(), (Configuration.NO_EVENT_DETAIL_URL, None))
