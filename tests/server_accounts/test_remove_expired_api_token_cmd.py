...
class RemoveExpiredAPITokenCmdTestCase(TestCase, EventAssertions):
    ...
    def test_remove_one_token(self, post_event):
        ...
        self.assertIn("Tokens marked for removal:", stdout)
        self.assertIn(token.hashed_key, stdout)
        self.assertEqual(stderr, "")
    ...
    def test_remove_token_by_user(self):
        ...
        expired_days = 3
        expired_date = datetime.today() - timedelta(days=(expired_days + 1))
        token1, _ = APIToken.objects.create_for_user(user, expiry=expired_date)
        token2, _ = APIToken.objects.create_for_user(user2, expiry=expired_date)
        token3, _ = APIToken.objects.create_for_user(user2, expiry=expired_date)

        stdout, stderr = self.call_command(
            "--after-days", expired_days, "--user-id", str(user2.pk)
        )

        self.assertIn("Tokens marked for removal:", stdout)
        self.assertIn(token2.hashed_key, stdout)
        self.assertIn(token3.hashed_key, stdout)
        self.assertEqual(stderr, "")
    ...
    def test_remove_token_dry_run(self):
        ...
        self.assertIn("Tokens marked for removal:", stdout)
        self.assertIn(token1.hashed_key, stdout)
        self.assertEqual(stderr, "")
        self.assertTrue(APIToken.objects.filter(id=token1.id).exists())
