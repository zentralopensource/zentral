...
class MDMEnterpriseAppsAPIViewsTestCase(TestCase, LoginCase, RequestCase):
    ...
    def test_create_enterprise_app_unknown_scheme(self):
        ...
        self.assertEqual(
            response.json(),
            {'package_uri': ["Unknown external resource URI scheme: 'ftp'"]}
        )
    ...
    def test_create_enterprise_app_hash_mismatch(self, download_s3_external_resource):
        _, artifact, _ = force_blueprint_artifact(
            artifact_type=Artifact.Type.ENTERPRISE_APP
        )
        package, _, _, _ = self._build_package()
        download_s3_external_resource.return_value = package
        self.set_permissions("mdm.add_enterpriseapp")
        response = self.post(reverse("mdm_api:enterprise_apps"),
                             data={"artifact": str(artifact.pk),
                                   "package_uri": "s3://yolo/fomo.pkg",
    ...
    @patch("zentral.utils.external_resources.download_s3_external_resource")
    def test_create_enterprise_app_different_product_id(self, download_s3_external_resource):
        _, artifact, _ = force_blueprint_artifact(
            artifact_type=Artifact.Type.ENTERPRISE_APP
        )
    ...
    @patch("zentral.utils.external_resources.download_s3_external_resource")
    def test_create_enterprise_app(self, download_s3_external_resource, post_event):
        ...
        self.assertEqual(
            ...
            {'id': str(ea_av.pk),
             ...
             'manifest': {'items': [{'assets': [{'kind': 'software-package',
                                                 ...
                                                 'md5s': [package_md5],
                                                 'sha256-size': package_size,
                                                 'sha256s': [package_sha256]}],
                                     'metadata': {'kind': 'software',
                                                  'title': 'test123',
                                                  'bundle-identifier': 'io.zentral.test123',
                                                  'bundle-version': '1.0'}}]},
    ...
    @patch("zentral.core.queues.backends.kombu.EventQueues.post_event")
    @patch("zentral.utils.external_resources.download_s3_external_resource")
    def test_update_enterprise_app(self, download_s3_external_resource, post_event):
        blueprint_artifact, artifact, (ea_av,) = force_blueprint_artifact(
            artifact_type=Artifact.Type.ENTERPRISE_APP
        )
        package, package_sha256, package_md5, package_size = self._build_package()
