...
from zentral.utils.storage import (_assembly_client, _request_client, abort_multipart_upload,
                                   ...
                                   file_storage_signed_url_expiration, generate_presigned_part,
                                   generate_presigned_put, list_multipart_parts, select_dist_storage,
...
class PresignedUploadTestCase(SimpleTestCase):
    ...
    @override_settings(STORAGES=S3_STORAGE)
    def test_generate_presigned_put_not_a_hex_digest(self):
        with self.assertRaises(ValueError) as cm:
            generate_presigned_put("turbo/uploads/test", 4, "application/gzip", "yolo", 900)
        self.assertEqual(cm.exception.args[0], "Not a hex digest: 'yolo'")

    @override_settings(STORAGES=S3_STORAGE)
    def test_generate_presigned_put_odd_length_hex(self):
        # half a byte is not a digest: the conversion refuses it here, so a malformed value from the
        # wire is a 400 and never reaches the storage
        with self.assertRaises(ValueError) as cm:
            generate_presigned_put("turbo/uploads/test", 4, "application/gzip", "abc", 900)
        self.assertEqual(cm.exception.args[0], "Not a hex digest: 'abc'")
...
class MultipartUploadTestCase(SimpleTestCase):
    ...
    def test_complete_supplies_the_whole_object_checksum(self):
        # the wire and the row carry the CRC as hex, S3 wants base64: the stub fails the call if the
        # conversion does not happen here
        storage, stub = self._stubbed("_assembly_client")
