...
The full inventory export has a `CSV` format too. Each table is a set of `.csv` files in a directory of the storage, like the Parquet export, and the task result carries the same manifest. Each file starts with the header row: you can open one file of a table on its own, and a reader that skips the header of each file reads all the files of a table together. A `NULL` and an empty string are both an empty field, because a CSV does not make the difference. Use the Parquet export when you need the types.
...
The link from a Santa target to one of its rules opens the rules of the configuration filtered on the target and the policy now. It was an anchor on the rule, and the rules of a configuration are paginated: the link landed on the first page, on rules that were not the one it was about, whenever the rule was on another page.

The page of a Santa machine has a Rules tab and a Path regexes tab now. The Rules tab gives one row for each target the machine has a rule for, on the server, on the device, or on both: the policy and the version of the server, what decided for the machine — a serial number, a primary user, a tag, or every machine for a rule with no scope field — the policy and the version of the device, and the state — on device, not yet on device, still on device, or skipped for a `CEL` rule the Santa version of the machine cannot evaluate. The candidates come from the statement the rule download reads, so the tab cannot disagree with what the next synchronization sends. The policy links to the rule that decided, and a target with more than one rule in the configuration gives their number next to the field that decided, with a link to all of them. The filters — target type, policy, voting, state — and the identifier search give the count of each value.

The Path regexes tab gives one row for each part of the two patterns Zentral sends: the patterns of the configuration itself, and the scoped path regex that won each pattern with the field that decided for the machine. Both tabs give what the machine gets with `Santa::Action::"viewEnrolledMachine"` alone — the policies, the versions and the patterns are the state of the machine — and need the permission of a rule or of an entry only to name it and to link to it.
...
The rules of a Santa configuration are a table now, with one row for each rule and two cells: the rule and its scope. The rule cell gives the policy, the ruleset, the type and the name of the target, the identifier of the target, the CEL expression, the custom message and the description. Each text carries its name, the way the sections of a scope do, because a rule can have three of them. The scope cell is the one the scoped client modes and the scoped path regexes use. Before, each rule was a panel with a properties table, and it listed every file behind the target: a cdhash that many bundles ship filled the page with the same name and the same path, over and over. The page of a target says what a target is, and the identifier links to it. The list reads the tags of its rules in one query, where each rule cost two more queries before.
...
`Turbo::Action::"createOneTimeJob"` takes the configuration as its resource, and `Turbo::Action::"updateOneTimeJob"` and `Turbo::Action::"deleteOneTimeJob"` take the schedule, which is a member of its configuration. All three carry the job in `context.job`, so one policy can name them all, and an update or delete policy can read `resource.job` as well. A policy can therefore allow one kind and refuse another, or grant one configuration and not another — neither of which the `turbo.add_onetimejob` permission can express. A policy needs no guard and no companion `forbid`: `when { context.job.kind == "sysdiagnose" }` grants that kind and nothing else, and the role still reaches the scheduling page, because Zentral only hides an action when no kind could be permitted. Every kind is covered, scripts and mSCP checks included, and the REST API is covered as well as the console.

A command uploads what it collects. The file does not travel in the result: the agent asks the new `/public/turbo/uploads/` endpoint for a destination, sends the file there, and reports the upload in the result. Zentral asks for the destination one time for each artifact of each run, and never in the configuration payload, which is cached and served again to every machine in scope.

When the storage backend is S3, the agent sends the file directly to the bucket and Zentral reads none of it. The size and the SHA-256 are signed into the destination as headers, so the storage itself refuses a body that does not match what the agent declared. Zentral forces version 4 signatures for this, because the version 2 signature that botocore still selects in `us-east-1`, and behind a custom endpoint, signs neither the length nor the digest. When the storage backend cannot sign a `PUT`, Zentral hosts the destination itself. Only S3 can sign one for now, so a Google Cloud Storage deployment always takes the hosted destination. The limit is then the `DATA_UPLOAD_MAX_MEMORY_SIZE` of the deployment, 10 MiB by default, and that is the number the configuration publishes: the agent must not learn a ceiling from us that the deployment does not honour. Raise it only together with the `client_max_body_size` of your reverse proxy. Use the hosted destination only in development, or for a small on-premises installation. The agent sees the same shape in the two cases.
...
The page of an enrolled machine lists the artifacts its jobs collected — one row for each, with its size, its state, and a download once the artifact is in. Two new PBAC actions gate that. `Turbo::Action::"viewJobUpload"` is an Admin, User and Viewer action: seeing that a job collected a sysdiagnose is an ordinary view. `Turbo::Action::"downloadJobUpload"` is Admin and User only, because reading a sysdiagnose hands over the data of the person who uses the machine. An artifact a user cannot see is absent from the page, not only without a link.
