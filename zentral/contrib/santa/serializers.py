...
class RuleSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if not policy.compatible_with_custom_msg:
            ...
            if error_field:
                raise serializers.ValidationError({error_field: "Cannot be set for this rule policy"})
...
class RuleUpdateSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        if not data["policy"].compatible_with_custom_msg and ("custom_msg" in data or "custom_url" in data):
            if data.get("custom_msg"):
