...
class Configuration(models.Model):
    ...
    event_detail_source = models.CharField(
        ...
        help_text="Where the block notification button comes from. Local configuration sends nothing, and "
                  ...
                  "button already sent to a machine: force a clean sync to do that. If the voting portal "
                  "becomes unavailable, Zentral removes the button."
    )
    ...
    def get_event_detail(self):
        ...
        if self.event_detail_source == self.EventDetailSource.VOTING_PORTAL:
            ...
            if not url:
                # the user portal was switched off after the configuration was saved. Removing the
                # button is better than leaving the machines with one that points at nothing.
