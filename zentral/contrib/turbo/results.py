...
# one parsed wire result: the resolved job/definition/per-machine tracker row plus the fields the batch
# needs. wire_ref is the raw entry echoed back as the TurboResultEvent payload. sort_key = (run time,
# batch index) so the latest run wins ties by batch order.
ParsedResult = namedtuple("ParsedResult", [
    "job", "definition", "job_machine", "kind", "version",
...
class ResultsBatch:
    ...
    def __init__(self, serial_number):
        ...
        self._job_statuses = {}      # (model, pk) -> tracker row, flushed once at commit
        self._result_version_at = {}  # (model, pk) -> ran_at that set last_result_version (latest run wins)
    ...
    def _apply_job_statuses(self):
        # rows may be OneTimeJobMachine or RecurringJobMachine — bulk_update can't mix models, so flush each
        by_model = {}
        for row in self._job_statuses.values():
            by_model.setdefault(type(row), []).append(row)
        for model, rows in by_model.items():
            model.objects.bulk_update(rows, ["last_result_version", "first_result_at", "last_result_at"])

    def _resolve_cc_statuses(self):
        # the latest verdict per check drives its stored status: N/A drops the row (see prune), any other
        # status is recorded. A repeated check thus reaches update_machine_statuses at most once.
        cc_statuses = []
        cc_out_of_scope = {}   # ComplianceCheck -> status_time of the N/A verdict
        max_cc_status_time = None
        for cc, (_, (status, status_time)) in self._cc_decisions.items():
            if status == Status.OUT_OF_SCOPE:
                cc_out_of_scope[cc] = status_time
            else:
    ...
    def _prune_cc_out_of_scope(self, cc_out_of_scope):
        if not cc_out_of_scope:
            return
        # a check that had a stored status and now reports N/A is a real transition to OUT_OF_SCOPE:
        # emit its per-check status-updated event (so probes see it clear, like munki) before dropping
        # the row — a check with no stored status was never in scope, so it is silently dropped
        status_time_by_cc_pk = {cc.pk: status_time for cc, status_time in cc_out_of_scope.items()}
        for machine_status in MachineStatus.objects.filter(
                serial_number=self.serial_number, compliance_check__in=list(cc_out_of_scope)):
            previous_status = Status(machine_status.status)
            if previous_status == Status.OUT_OF_SCOPE:
                continue
            definition = self._cc_definitions[machine_status.compliance_check_id]
            status_time = status_time_by_cc_pk.get(machine_status.compliance_check_id)
            definition.compliance_check.loaded_compliance_check.build_status_updated_event(
                definition, self.serial_number, Status.OUT_OF_SCOPE, status_time, previous_status).post()
        MachineStatus.objects.filter(
            serial_number=self.serial_number, compliance_check__in=list(cc_out_of_scope)).delete()
