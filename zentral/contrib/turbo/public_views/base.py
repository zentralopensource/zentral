    ...
    def get_json_data(self, request):
        ...
        if request.META.get("HTTP_CONTENT_ENCODING") == "gzip":
            try:
                ...
                raise WireError("invalid_gzip")
        try:
            data = json.loads(payload)
        except ValueError:
            raise WireError("invalid_json")
        # every agent POST body is a JSON object; anything else (list, string, …) is malformed → 400,
        # not an AttributeError deep in a do_post()
