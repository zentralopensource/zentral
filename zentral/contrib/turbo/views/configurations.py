...
class ConfigurationView(PermissionRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # only load the enrollments (secrets, distributor info) for a user allowed to see them — the
        # template hides the section otherwise, but the view should not fetch what it must not show
        if self.request.user.has_perm("turbo.view_enrollment"):
            enrollments = []
            enrollment_count = 0
        ...
        # one decision for each row, because the rows have different jobs
