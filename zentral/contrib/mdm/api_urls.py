from django.urls import path

from .api_views import (
                        ACMEIssuerDetail,
                        ACMEIssuerList,
                        ArtifactDetail,
                        ArtifactList,
                        BlockEnrolledDevice,
                        BlueprintArtifactDetail,
                        BlueprintArtifactList,
                        BlueprintDetail,
                        BlueprintList,
                        CertAssetDetail,
                        CertAssetList,
                        DataAssetDetail,
                        DataAssetList,
                        DeclarationDetail,
                        DeclarationList,
                        DEPDeviceDetail,
                        DEPDeviceList,
                        DEPEnrollmentCustomViewDetail,
                        DEPEnrollmentCustomViewList,
                        DEPEnrollmentDetail,
                        DEPEnrollmentList,
                        DEPVirtualServerBetaTokensView,
                        DEPVirtualServerDetail,
                        DEPVirtualServerList,
                        DEPVirtualServerSyncDevicesView,
                        DisownDEPDevice,
                        EnrolledDeviceAdminPassword,
                        EnrolledDeviceArtifactList,
                        EnrolledDeviceCommand,
                        EnrolledDeviceCommandList,
                        EnrolledDeviceDeviceLockPin,
                        EnrolledDeviceFileVaultPRK,
                        EnrolledDeviceList,
                        EnrolledDeviceRecoveryPassword,
                        EnrolledUserArtifactList,
                        EnrolledUserCommand,
                        EnrolledUserCommandList,
                        EnrollmentCustomViewDetail,
                        EnrollmentCustomViewList,
                        EnterpriseAppDetail,
                        EnterpriseAppList,
                        EraseEnrolledDevice,
                        FileVaultConfigDetail,
                        FileVaultConfigList,
                        ForceInstallEnrolledDeviceArtifact,
                        ForceInstallEnrolledUserArtifact,
                        LocationAssetList,
                        LocationDetail,
                        LocationList,
                        LockEnrolledDevice,
                        OTAEnrollmentDetail,
                        OTAEnrollmentList,
                        PackageDetail,
                        PackageList,
                        ProfileDetail,
                        ProfileList,
                        ProvisioningProfileDetail,
                        ProvisioningProfileList,
                        PushCertificateDetail,
                        PushCertificateList,
                        RecoveryPasswordConfigDetail,
                        RecoveryPasswordConfigList,
                        SCEPIssuerDetail,
                        SCEPIssuerList,
                        SendCustomEnrolledDeviceCommand,
                        SoftwareUpdateEnforcementDetail,
                        SoftwareUpdateEnforcementList,
                        StoreAppDetail,
                        StoreAppList,
                        SyncSoftwareUpdatesView,
                        UnblockEnrolledDevice,
)

app_name = "mdm_api"
urlpatterns = [
    path('acme_issuers/', ACMEIssuerList.as_view(), name="acme_issuers"),
    path('acme_issuers/<uuid:pk>/', ACMEIssuerDetail.as_view(), name="acme_issuer"),
    path('artifacts/', ArtifactList.as_view(), name="artifacts"),
    path('artifacts/<uuid:pk>/', ArtifactDetail.as_view(), name="artifact"),
    path('blueprints/', BlueprintList.as_view(), name="blueprints"),
    path('blueprints/<int:pk>/', BlueprintDetail.as_view(), name="blueprint"),
    path('blueprint_artifacts/', BlueprintArtifactList.as_view(), name="blueprint_artifacts"),
    path('blueprint_artifacts/<int:pk>/', BlueprintArtifactDetail.as_view(), name="blueprint_artifact"),
    path('cert_assets/', CertAssetList.as_view(), name="cert_assets"),
    path('cert_assets/<uuid:artifact_version_pk>/', CertAssetDetail.as_view(), name="cert_asset"),
    path('data_assets/', DataAssetList.as_view(), name="data_assets"),
    path('data_assets/<uuid:artifact_version_pk>/', DataAssetDetail.as_view(), name="data_asset"),
    path('declarations/', DeclarationList.as_view(), name="declarations"),
    path('declarations/<uuid:artifact_version_pk>/', DeclarationDetail.as_view(), name="declaration"),
    path('enterprise_apps/', EnterpriseAppList.as_view(), name="enterprise_apps"),
    path('enterprise_apps/<uuid:artifact_version_pk>/', EnterpriseAppDetail.as_view(), name="enterprise_app"),
    path('filevault_configs/', FileVaultConfigList.as_view(), name="filevault_configs"),
    path('filevault_configs/<int:pk>/', FileVaultConfigDetail.as_view(), name="filevault_config"),
    path('locations/', LocationList.as_view(), name="locations"),
    path('locations/<int:pk>/', LocationDetail.as_view(), name="location"),
    path('location_assets/', LocationAssetList.as_view(), name="location_assets"),
    path('ota_enrollments/', OTAEnrollmentList.as_view(), name="ota_enrollments"),
    path('ota_enrollments/<int:pk>/', OTAEnrollmentDetail.as_view(), name="ota_enrollment"),
    path('packages/', PackageList.as_view(), name="packages"),
    path('packages/<uuid:pk>/', PackageDetail.as_view(), name="package"),
    path('profiles/', ProfileList.as_view(), name="profiles"),
    path('profiles/<uuid:artifact_version_pk>/', ProfileDetail.as_view(), name="profile"),
    path('provisioning_profiles/', ProvisioningProfileList.as_view(), name="provisioning_profiles"),
    path('provisioning_profiles/<uuid:artifact_version_pk>/', ProvisioningProfileDetail.as_view(),
         name="provisioning_profile"),
    path('push_certificates/', PushCertificateList.as_view(), name="push_certificates"),
    path('push_certificates/<int:pk>/', PushCertificateDetail.as_view(), name="push_certificate"),
    path('recovery_password_configs/', RecoveryPasswordConfigList.as_view(), name="recovery_password_configs"),
    path('recovery_password_configs/<int:pk>/', RecoveryPasswordConfigDetail.as_view(),
         name="recovery_password_config"),
    path('scep_issuers/', SCEPIssuerList.as_view(), name="scep_issuers"),
    path('scep_issuers/<uuid:pk>/', SCEPIssuerDetail.as_view(), name="scep_issuer"),
    path('software_update_enforcements/', SoftwareUpdateEnforcementList.as_view(),
         name="software_update_enforcements"),
    path('software_update_enforcements/<int:pk>/', SoftwareUpdateEnforcementDetail.as_view(),
         name="software_update_enforcement"),
    path('store_apps/', StoreAppList.as_view(), name="store_apps"),
    path('store_apps/<uuid:artifact_version_pk>/', StoreAppDetail.as_view(), name="store_app"),

    path('dep/virtual_servers/', DEPVirtualServerList.as_view(), name="dep_virtual_servers"),
    path('dep/virtual_servers/<int:pk>/', DEPVirtualServerDetail.as_view(), name="dep_virtual_server"),
    path('dep/virtual_servers/<int:pk>/sync_devices/',
         DEPVirtualServerSyncDevicesView.as_view(), name="dep_virtual_server_sync_devices"),
    path('dep/virtual_servers/<int:pk>/beta_tokens/',
         DEPVirtualServerBetaTokensView.as_view(), name="dep_virtual_server_beta_tokens"),
    path('dep/devices/', DEPDeviceList.as_view(), name="dep_devices"),
    path('dep/devices/<int:pk>/', DEPDeviceDetail.as_view(), name="dep_device"),
    path('dep/devices/<int:pk>/disown/', DisownDEPDevice.as_view(), name="disown_dep_device"),
    path('devices/', EnrolledDeviceList.as_view(), name="enrolled_devices"),
    path('devices/<int:pk>/block/', BlockEnrolledDevice.as_view(), name="block_enrolled_device"),
    path('devices/<int:pk>/unblock/', UnblockEnrolledDevice.as_view(), name="unblock_enrolled_device"),
    path('devices/<int:pk>/erase/', EraseEnrolledDevice.as_view(), name="erase_enrolled_device"),
    path('devices/<int:pk>/lock/', LockEnrolledDevice.as_view(), name="lock_enrolled_device"),
    path('devices/<int:pk>/send_custom_command/', SendCustomEnrolledDeviceCommand.as_view(),
         name="send_custom_enrolled_device_command"),
    path('devices/<int:pk>/filevault_prk/', EnrolledDeviceFileVaultPRK.as_view(),
         name="enrolled_device_filevault_prk"),
    path('devices/<int:pk>/recovery_password/', EnrolledDeviceRecoveryPassword.as_view(),
         name="enrolled_device_recovery_password"),
    path('devices/<int:pk>/admin_password/', EnrolledDeviceAdminPassword.as_view(),
         name="enrolled_device_admin_password"),
    path('devices/<int:pk>/device_lock_pin/', EnrolledDeviceDeviceLockPin.as_view(),
         name="enrolled_device_device_lock_pin"),
    path('devices/<int:pk>/artifacts/<uuid:artifact_pk>/force_install/',
         ForceInstallEnrolledDeviceArtifact.as_view(),
         name="force_install_enrolled_device_artifact"),
    path('devices/artifacts/', EnrolledDeviceArtifactList.as_view(),
         name="enrolled_device_artifacts"),
    path('devices/commands/', EnrolledDeviceCommandList.as_view(), name="enrolled_device_commands"),
    path('devices/commands/<uuid:uuid>/', EnrolledDeviceCommand.as_view(), name="enrolled_device_command"),
    path('users/<int:pk>/artifacts/<uuid:artifact_pk>/force_install/',
         ForceInstallEnrolledUserArtifact.as_view(),
         name="force_install_enrolled_user_artifact"),
    path('users/artifacts/', EnrolledUserArtifactList.as_view(),
         name="enrolled_user_artifacts"),
    path('users/commands/', EnrolledUserCommandList.as_view(), name="enrolled_user_commands"),
    path('users/commands/<uuid:uuid>/', EnrolledUserCommand.as_view(), name="enrolled_user_command"),
    path('software_updates/sync/',
         SyncSoftwareUpdatesView.as_view(), name="sync_software_updates"),
    path('dep_enrollments/', DEPEnrollmentList.as_view(), name="dep_enrollments"),
    path('dep_enrollments/<int:pk>/', DEPEnrollmentDetail.as_view(), name="dep_enrollment"),
    path('enrollment_custom_views/', EnrollmentCustomViewList.as_view(),
         name="enrollment_custom_views"),
    path('enrollment_custom_views/<uuid:pk>/', EnrollmentCustomViewDetail.as_view(),
         name="enrollment_custom_view"),
    path('dep_enrollment_custom_views/', DEPEnrollmentCustomViewList.as_view(),
         name="dep_enrollment_custom_views"),
    path('dep_enrollment_custom_views/<uuid:pk>/', DEPEnrollmentCustomViewDetail.as_view(),
         name="dep_enrollment_custom_view")
]
