...
def update_realm_tags(realm, request):
    query = (
      ...
      "  SELECT pk, user_pk FROM sorted_enrollment_sessions WHERE row_number=1 and user_pk IS NOT NULL"
      ...
      "    JOIN enrollment_sessions es ON (es.enrolled_device_id=ed.id)"
      "    JOIN realms_realmuser u ON (u.uuid=es.user_pk)"
