...
from .models import TargetArtifact
...
class MetricsView(BasePrometheusMetricsView):
    ...
    def populate_target_artifacts_buckets(self):
        tag = Gauge(
            ...
            ["name", "type", "channel", "version", "status", "present", "le"],
            registry=self.registry,
        )
        with connection.cursor() as cursor:
            cursor.execute(
                "with all_target_artifacts as ("
                "  select a.name, a.type, a.channel, av.version, da.status,"
                "  date_part('days', now() - ed.last_seen_at) as age"
                "  from mdm_artifact a"
                "  join mdm_artifactversion av on (av.artifact_id = a.id)"
                "  join mdm_deviceartifact da on (da.artifact_version_id = av.id)"
                "  join mdm_enrolleddevice ed on (ed.id = da.enrolled_device_id)"
                "  union all"
                "  select a.name, a.type, a.channel, av.version, ua.status,"
                "  date_part('days', now() - eu.last_seen_at) as age"
                "  from mdm_artifact a"
                "  join mdm_artifactversion av on (av.artifact_id = a.id)"
                "  join mdm_userartifact ua on (ua.artifact_version_id = av.id)"
                "  join mdm_enrolleduser eu on (eu.id = ua.enrolled_user_id)"
                ") select name, type, channel, version, status,"
                ...
                'count(*) as "+Inf" '
                "from all_target_artifacts "
                "group by name, type, channel, version, status"
            )
            columns = [c.name for c in cursor.description]
            for row in cursor.fetchall():
                row_d = dict(zip(columns, row))
                status = row_d["status"]
                try:
                    present = str(TargetArtifact.Status(status).present).lower()
                except ValueError:
                    present = "_"
                for le in ("1", "7", "14", "30", "45", "90", "+Inf"):
                    tag.labels(
                        name=row_d["name"],
                        type=row_d["type"],
                        channel=row_d["channel"],
                        version=str(row_d["version"]),
                        status=status,
                        present=present,
                        le=le,
                    ).set(row_d[le])
