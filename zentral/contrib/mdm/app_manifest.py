...
def read_distribution_info(tmp_filepath):
    ...
    for bundle in installer_script_elm.findall(".//bundle"):
        try:
            pkg_data["bundles"].append({k: bundle.attrib[v]
    ...
    if title_elm is not None and title_elm.text:
        name = title_elm.text
    return name, [Platform.MACOS], pkg_data
...
def read_ipa_info(tmp_filepath):
    ...
    return name, platforms, pkg_data
...
def build_manifest_metadata(name, pkg_data):
    """Assemble the manifest.items[0].metadata block from reader output.

    Pops `platform_identifier` from pkg_data. Raises ValueError if any required
    Apple-spec key (kind/title/bundle-identifier/bundle-version) is missing.
    """
    metadata = {
        "kind": "software",
        "title": name,
        "bundle-identifier": pkg_data["product_id"],
        "bundle-version": pkg_data["product_version"],
    }
    ...
    platform_identifier = pkg_data.pop("platform_identifier", None)
...
def read_package_info(package_file, compute_sha256=False):
    ...
    try:
        name, platforms, pkg_data = file_opener(tmp_filepath)
    finally:
        if cleanup_tmp_file:
    ...
    metadata = build_manifest_metadata(name, pkg_data)
    ...
    pkg_data["manifest"] = manifest
    pkg_data["package_size"] = package_size
    if compute_sha256:
        pkg_data["package_sha256"] = package_sha256
    return name, platforms, pkg_data
