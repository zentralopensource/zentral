...
MAC_VERSION_RE = re.compile(
    r"^(?P<name>.*) (?P<major>[0-9]{1,3})\."
    "(?P<minor>[0-9]{1,3})(?:\."
    "(?P<patch>[0-9]{1,3}))?(?: \((?P<build>.*)\))?$"
)

LINUX_VERSION_RE = re.compile(
    r"^(?P<name>[^0-9]*) (?P<major>[0-9]{1,3})\."
    "(?P<minor>[0-9]{1,3})(?:\."
    "(?P<patch>[0-9]{1,3}))?.*$"
)

WINDOWS_MODERN_VERSION_RE = re.compile(
    r"^(?P<name>.*) (?P<major>[0-9]{1,4}) (.*) (?P<build>build [0-9]{1,4})"
)

PROCESSOR_RE = re.compile(
    r"(?P<cpu_brand>.*) \((?:(?P<cpu_physical_cores>[0-9]+) core )?"
    "(?P<cpus>[0-9]+) processor\)$"
)

WINDOWS_LEGACY_CLIENT_DICT = {
    "5.1.2600": {"name": "Windows XP", "major": 5, "build": "Service Pack 3"},
    "6.0.6000": {"name": "Windows Vista", "major": 6},
    "6.0.6001": {"name": "Windows Vista", "major": 6, "build": "Service Pack 1"},
    "6.0.6002": {"name": "Windows Vista", "major": 6, "build": "Service Pack 2"},
    "6.1.7600": {"name": "Windows", "major": 7},
    "6.1.7601": {"name": "Windows", "major": 7, "build": "Service Pack 1"},
    "6.2.9200": {"name": "Windows", "major": 8},
    "6.3.9200": {"name": "Windows", "major": 8, "minor": 1},
    "6.3.9600": {"name": "Windows", "major": 8, "minor": 1, "build": "Update 1"},
}

WINDOWS_LEGACY_SERVER_DICT = {
    "5.2.3790": {"name": "Windows Server", "major": 2003},
    "6.0.6001": {"name": "Windows Server", "major": 2008},
    "6.0.6002": {"name": "Windows Server", "major": 2008, "build": "Service Pack 2"},
    "6.0.6003": {
        "name": "Windows Server",
        "major": 2008,
        "build": "Service Pack 2, Rollup KB4489887",
    },
    "6.1.7600": {"name": "Windows Server", "major": 2008, "build": "R2"},
    "6.1.7601": {
        "name": "Windows Server",
        "major": 2008,
        "build": "R2, Service Pack 1",
    },
    "6.1.8400": {"name": "Windows Home Server", "major": 2011},
    "6.2.9200": {"name": "Windows Server", "major": 2012},
    "6.3.9600": {"name": "Windows Server", "major": 2012, "build": "R2"},
}
...
class InventoryClient(BaseInventory):
    ...
    def __init__(self, config_d):
        ...
        self.session.headers.update(
            {"user-agent": "zentral/0.0.1", "accept": "application/json"}
        )
        ...
        self.session.mount(
            self.base_api_url, requests.adapters.HTTPAdapter(max_retries=max_retries)
        )
    ...
    def _make_get_query(self, path, **params):
        ...
        if r.status_code != requests.codes.ok:
            raise InventoryError(
                "Watchman API HTTP response status code %s" % r.status_code
            )
        return r.json()
    ...
    def _computers(self):
        return self._make_paginated_get_query(
            "/computers", **{"expand[]": "computer.plugin_results"}
        )
    ...
    def _group_machine_links_from_plugin_slug(self, slug):
        ll = []
        for anchor_text, url_tmpl in (
            ("Plugin History", "{}/computers/%MACHINE_SNAPSHOT_REFERENCE%/{}/history"),
        ):
            ll.append(
                {
                    "anchor_text": anchor_text,
        ...
        for c in self._computers():
            ...
            if serial_number in machines:
                ...
                if machines[serial_number]["created_at"] > created_at:
                    logger.warning(
                        "Instance %s of Computer %s ignored", machine_id, serial_number
                    )
                    continue
                else:
                    logger.warning(
                        "Instance %s of Computer %s ignored",
            ...
            if platform == "linux" or platform == "windows":
                ...
                logger.warning(
                    "Unknown platform %s for computer %s", platform, machine_id
                )
                continue

            # serial number, reference
            ct = {
                "reference": str(machine_id),
            ...
            for plugin_result in c.get("plugin_results", []):
                if plugin_result["status"] == "warning":
                    ...
                    groups.append(
                        {
                            "name": "{} - Warning".format(plugin_result["name"]),
                            "reference": slug,
                            "machine_links": self._group_machine_links_from_plugin_slug(
                                slug
                            ),
                        }
                    )
            ...
            if platform == "mac":
                ...
                m = WINDOWS_MODERN_VERSION_RE.match(c["os_version"])

            if m and (platform == "mac" or platform == "linux"):
            ...
            system_info.update(
                self._system_info_update_dict_from_processor(c["processor"])
            )
